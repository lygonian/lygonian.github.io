...
(function () {
  ...
  // Native Scrollregionen brauchen nur bei echtem Überlauf einen Tabstopp.
  // Ohne JavaScript bleiben tabindex und die mobilen Hinweise im HTML erhalten.
  var scrollRegions = [].slice.call(document.querySelectorAll(".diagram-scroll, .table-scroll"));
  if (scrollRegions.length) {
    ...
    function scheduleScrollRegions() {
      if (!scrollFrame) scrollFrame = window.requestAnimationFrame(updateScrollRegions);
    }
    scheduleScrollRegions();
    window.addEventListener("resize", scheduleScrollRegions);
    window.addEventListener("load", scheduleScrollRegions, { once: true });
    if (document.fonts && document.fonts.ready) document.fonts.ready.then(scheduleScrollRegions);
    if ("ResizeObserver" in window) {
      var scrollObserver = new ResizeObserver(scheduleScrollRegions);
      scrollRegions.forEach(function (region) { scrollObserver.observe(region); });
    }
  }

  // Projektgruppen: echte Anker ohne JS, umschaltbare Filter mit teilbarer URL.
  var projectFilters = document.querySelector("[data-project-filters]");
  if (projectFilters) {
    document.querySelector("[data-filter-instructions]").hidden = false;
    var filterBanners = [].slice.call(projectFilters.querySelectorAll("[data-project-category]"));
    var projectGroups = [].slice.call(document.querySelectorAll("[data-project-group]"));
    var filterStatus = document.querySelector("[data-project-status]");
    var selectedProjectGroup = "";

    function validProjectGroup(value) {
      return filterBanners.some(function (banner) { return banner.dataset.projectCategory === value; });
    }
    function projectGroupFromUrl() {
      var requested = new URL(window.location.href).searchParams.get("filter");
      if (validProjectGroup(requested)) return requested;
      var anchor;
      try { anchor = decodeURIComponent(window.location.hash.slice(1)); } catch (_) { return ""; }
      if (validProjectGroup(anchor)) return anchor;
      var target = anchor && document.getElementById(anchor);
      var group = target && target.closest("[data-project-group]");
      return group && validProjectGroup(group.dataset.projectGroup) ? group.dataset.projectGroup : "";
    }
    function renderProjectFilter(value) {
      selectedProjectGroup = value;
      projectFilters.classList.toggle("has-selection", Boolean(value));
      filterBanners.forEach(function (banner) {
        var active = banner.dataset.projectCategory === value;
        banner.setAttribute("aria-pressed", String(active));
        banner.querySelector(".project-banner-cta > span").textContent = active ? "Filter aufheben" : "Projekte anzeigen";
        banner.querySelector(".project-banner-arrow").textContent = active ? "✓" : "→";
      });
      var visibleCount = 0;
      var label = "Alle Projekte";
      projectGroups.forEach(function (group) {
        group.hidden = Boolean(value) && group.dataset.projectGroup !== value;
        if (!group.hidden) visibleCount += group.querySelectorAll("[data-project-entry]").length;
        if (group.dataset.projectGroup === value) label = group.querySelector("h2").textContent;
      });
      filterStatus.textContent = label + " · " + visibleCount + " Arbeiten";
    }
    function selectProjectFilter(value) {
      var next = value === selectedProjectGroup ? "" : value;
      var url = new URL(window.location.href);
      if (next) url.searchParams.set("filter", next);
      else url.searchParams.delete("filter");
      url.hash = "";
      window.history.pushState(null, "", url);
      renderProjectFilter(next);
      if (window.matchMedia && window.matchMedia("(max-width: 760px)").matches) {
        var results = document.getElementById("projektliste");
        if (results) {
          results.setAttribute("tabindex", "-1");
          results.focus({ preventScroll: true });
          results.scrollIntoView({ behavior: reduce ? "instant" : "smooth", block: "start" });
        }
      }
    }
    function enhanceFilter(control, value) {
      control.setAttribute("role", "button");
      control.setAttribute("aria-controls", "projektliste");
      control.addEventListener("click", function (event) {
        if (event.button !== 0 || event.ctrlKey || event.metaKey || event.shiftKey || event.altKey) return;
        event.preventDefault();
        selectProjectFilter(value);
      });
      control.addEventListener("keydown", function (event) {
        if (event.key === " ") { event.preventDefault(); control.click(); }
      });
    }
    filterBanners.forEach(function (banner) { enhanceFilter(banner, banner.dataset.projectCategory); });
    window.addEventListener("popstate", function () { renderProjectFilter(projectGroupFromUrl()); });
    window.addEventListener("hashchange", function () { renderProjectFilter(projectGroupFromUrl()); });
    renderProjectFilter(projectGroupFromUrl());
  }
  ...
  // Der Hauptstrahl ist eigenständiges Artwork, seine Geometrie bleibt aber
  // an die beiden Chloroplast-Routenüberschriften gekoppelt. Im früheren
  // verdichteten Layout lagen deren Mittelpunkte 788 px auseinander. Drei
  // Halbwellen auf dem neuen Abstand von 888 px ergeben eine volle
  // Wellenlänge von 592 px: exakt 75 % des vorherigen Abstands.
  var homeWave = homeMain && homeMain.querySelector(".home-route-artwork");
  var homeWaveFrame = 0;

  function drawHomeWave() {
    homeWaveFrame = 0;
    if (!homeWave || window.innerWidth < 1024) return;

    var dnaHeading = homeMain.querySelector(".route-intro--domains .route-blob--heading");
    var workHeading = homeMain.querySelector(".route-intro--work .route-blob--heading");
    if (!dnaHeading || !workHeading) return;
    ...
    var centerY = function (element) {
      ...
      return rect.top - mainRect.top + rect.height / 2;
    };
    var dnaZero = centerY(dnaHeading);
    var workZero = centerY(workHeading);
    var halfWave = (workZero - dnaZero) / 3;
    if (!isFinite(halfWave) || halfWave < 120) return;

    var amplitude = Math.min(width * 0.31, 310);
    var controlOffset = amplitude / 0.75;
    var firstZero = dnaZero;
    while (firstZero > -halfWave) firstZero -= halfWave;
    var segmentIndex = Math.round((firstZero - dnaZero) / halfWave);
    var pathData = "M " + centerX.toFixed(1) + " " + firstZero.toFixed(1);

    for (var y = firstZero; y < height + halfWave; y += halfWave) {
      var direction = Math.abs(segmentIndex % 2) === 0 ? 1 : -1;
      var controlX = centerX + direction * controlOffset;
      var endY = y + halfWave;
      pathData += " C " +
        controlX.toFixed(1) + " " + (y + halfWave / 3).toFixed(1) + ", " +
        controlX.toFixed(1) + " " + (y + halfWave * 2 / 3).toFixed(1) + ", " +
        centerX.toFixed(1) + " " + endY.toFixed(1);
      segmentIndex += 1;
    }

    homeWave.setAttribute("viewBox", "0 0 " + width + " " + height);
    homeWave.setAttribute("preserveAspectRatio", "none");
    // Der Lichtpfad bleibt hinter dem Titel. Seine helle Kontur darf den
    // Kontrast des verbindlichen Textverlaufs nicht örtlich aufheben.
    var title = homeMain.querySelector(".home-hero h1");
    var mask = homeWave.querySelector("#home-title-clearance");
    var maskWorld = homeWave.querySelector(".home-title-mask-world");
    var maskCutout = homeWave.querySelector(".home-title-mask-cutout");
    if (title && mask && maskWorld && maskCutout) {
      var titleRect = title.getBoundingClientRect();
      mask.setAttribute("width", width);
      mask.setAttribute("height", height);
      maskWorld.setAttribute("width", width);
      maskWorld.setAttribute("height", height);
      maskCutout.setAttribute("x", titleRect.left - mainRect.left - 24);
      maskCutout.setAttribute("y", titleRect.top - mainRect.top - 24);
      maskCutout.setAttribute("width", titleRect.width + 48);
      maskCutout.setAttribute("height", titleRect.height + 48);
      maskCutout.setAttribute("rx", "24");
    }
    [].slice.call(homeWave.querySelectorAll("path")).forEach(function (path) {
      path.setAttribute("d", pathData);
    });
  }

  function scheduleHomeWave() {
    if (!homeWaveFrame) homeWaveFrame = window.requestAnimationFrame(drawHomeWave);
  }

  if (homeWave) {
    scheduleHomeWave();
    window.addEventListener("load", scheduleHomeWave, { once: true });
    window.addEventListener("resize", scheduleHomeWave);
    if (document.fonts && document.fonts.ready) document.fonts.ready.then(scheduleHomeWave);
    if ("ResizeObserver" in window) new ResizeObserver(scheduleHomeWave).observe(homeMain);
  }
